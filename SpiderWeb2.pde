class SpiderWeb2 extends RunableSketch {
  ...
  SpiderWeb2() {
    ...
    circs = new float[2][number_circ];
    
    nodes = new Vector[number_radii][number_circ];

    for (int i = 0; i < number_circ; i++) {
      circs[0][i] = random(0,width/2);
      circs[1][i] = random(0,height/2);
    }
      
    circs[0] = sort(circs[0]);
    circs[1] = sort(circs[1]);
      
    for (int i = 0; i < number_radii; i++) {
      float s = sin(radii[i]);
      float c = cos(radii[i]);
      for (int j = 0; j < number_circ; j++) {
        nodes[i][j] = new Vector((width/2)+circs[0][j]*c,(height/2)+circs[1][j]*s);
      }
    }   
  }
